...
// TestFindModelProvider_IgnoresDisabledProvider is the regression test for
// findModelProvider routing a subagent to a provider the user has explicitly
// disabled. Without an override it must skip disabled providers entirely
// (even when no other provider offers the model), and with providerOverride
// naming the disabled provider directly it must still refuse the match — the
// override path never checked Disable at all.
func TestFindModelProvider_IgnoresDisabledProvider(t *testing.T) {
	t.Parallel()
	...
	cfg.Config().Providers.Set("disabled-provider", config.ProviderConfig{
	...
	t.Run("no_override_skips_disabled_provider", func(t *testing.T) {
		t.Parallel()

		_, _, ok := coord.findModelProvider("only-here-model", "")
		require.False(t, ok, "a disabled provider's model must not be found even when no other provider offers it")
	})

	t.Run("override_naming_disabled_provider_still_refused", func(t *testing.T) {
		t.Parallel()

		_, _, ok := coord.findModelProvider("only-here-model", "disabled-provider")
		require.False(t, ok, "an explicit providerOverride must not bypass the provider's Disable flag")
	})
}
