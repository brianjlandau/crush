...
func (c *coordinator) findModelProvider(modelID, providerOverride string) (config.ProviderConfig, catwalk.Model, bool) {
	// EnabledProviders already filters out providers with Disable set, so
	// neither the override lookup nor the fallback scan below can match a
	// disabled provider.
	providers := c.cfg.Config().EnabledProviders()
	if providerOverride != "" {
		for _, p := range providers {
			if p.ID != providerOverride {
				continue
			}
