...
// dispatcher enum, dispatch lookup, @-mention completions, and the
// @-rewrite all derive from — so it can be neither auto-selected by the main
// agent nor invoked manually. Enabling always writes workspace scope only;
// it never rewrites whichever scope actually disabled the name. Instead it
// records the name in options.enabled_subagents, which
...
func (w *AppWorkspace) SetSubagentDisabled(name string, disabled bool) error {
	...
	if err := w.store.SetConfigField(config.ScopeWorkspace, "options.disabled_subagents", nextDisabled); err != nil {
		return err
	}
	if err := w.store.SetConfigField(config.ScopeWorkspace, "options.enabled_subagents", nextEnabled); err != nil {
		return err
	}
	w.reloadSubagents()
	return nil
