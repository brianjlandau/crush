...
// TestSetDefaults_UserConfiguredPathsWinPrecedence verifies that a
// user-preconfigured SubagentsPaths entry ends up last in the slice after
// setDefaults runs. subagents.DiscoverWithStates documents that later paths
// win on a name collision, so a user's explicit path must be positioned
// after the built-in global/project default dirs, not before them.
func TestSetDefaults_UserConfiguredPathsWinPrecedence(t *testing.T) {
	t.Parallel()

	userPath := "/custom/user/path"
	cfg := &Config{
		Options: &Options{
			SubagentsPaths: []string{userPath},
		},
	}
	workingDir := t.TempDir()
	...
	require.NotEmpty(t, cfg.Options.SubagentsPaths)
	require.Equal(t, userPath, cfg.Options.SubagentsPaths[len(cfg.Options.SubagentsPaths)-1],
		"user-configured path must be last so it wins over built-in defaults on a name collision; got %v",
		cfg.Options.SubagentsPaths)
}
